module Item = {
  ...
  let make = (~value, ~op: Shape.Stringman.t) => {
    ...
    <div
      onClick
      tabIndex=0
      className="col-span-12 md:col-span-6 lg:col-span-4 xl:col-span-3 p-4 flex gap-2 border border-main rounded-box cursor-pointer hover:bg-primary/5 active:bg-primary/10 transit active:scale-105">
      <div className="flex flex-col grow gap-2">
        <p className="text-80 text-sm"> {op.title->React.string} </p>
        <p className="text-xl"> {text->React.string} </p>
      </div>
    </div>
  }
}
  ...
  <div className="flex flex-col md:flex-row size-full bg-base-100">
    ...
    <div
      ...
      <div className="col-span-12 center flex-col">
        <div className="text-7xl font-black text-primary mb-8"> {"Stringman"->React.string} </div>
        <label
